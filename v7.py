...
client = OpenAI(api_key=os.getenv("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"))


  # or hard-code your key (not recommended for production)
...
if __name__ == "__main__":
    city_center = (40, -74)
    city_radius = 15
    drivers = [Driver(f"D{i:05d}", city_center, city_radius) for i in range(200)]
    passengers = [Passenger(f"P{i:05d}", city_center, city_radius) for i in range(350)]
    city = City(city_center, city_radius, drivers, passengers)
    city.run_simulation(duration_seconds=60, request_interval=2)
